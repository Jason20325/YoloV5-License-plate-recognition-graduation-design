...
if __name__ == '__main__':
    # 创造窗口
    window = sg.Window('车牌识别界面', lay.layout, size=lay.win_size, resizable=True,
                       auto_size_text=True,
    ...
    while True:
        ...
        if event is None:  # 如果用户关闭窗口
            break
        if event == 'load':
            file_path1 = lay.get_img_path(lay.path1)
            img = Image.open(file_path1)
            img = img.resize(lay.image_size)
            base = lay.image_to_base64(img)
            # window['qian'].update(image_filename=path, image_size=image_size)
            window['qian'].update(image_data=base, image_size=lay.image_size)
        if event == 'do':
            ...
            with torch.no_grad():
                if opt.update:  # update all models (to fix SourceChangeWarning)
                    ...
                    detect()
            file_path2 = lay.get_img_path(lay.path2)
            img = Image.open(file_path2)
            img = img.resize(lay.image_size)
            base = lay.image_to_base64(img)
            window['hou'].update(image_data=base, image_size=lay.image_size)
    window.close()
